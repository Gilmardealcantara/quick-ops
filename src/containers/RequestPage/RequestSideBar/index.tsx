...
import { SideBarDataState } from '../../../store/modules/sidebarData/types';
import { HeaderDataState } from '../../../store/modules/headerData/types';
import { loadSideBarData } from '../../../store/modules/sidebarData/actions'
...
const RequestSideBar = () => {
    ...
    const { header, sidebar } = useSelector<ApplicationState, LocalState>(state =>
        ({ header: state.headerData, sidebar: state.sidebarData }));

    useEffect(() => { dispatch(loadSideBarData()); }, []);
    ...
    return (
        <div className="request-side-bar">
            ...
            <div className="request-side-bar-coins">
                {sidebar.data.filter(x => x.declared !== 0).map(item => <CoinItem key={item.currency.id} sidebarData={item} />)}
            </div>
        </div>
    );
}
